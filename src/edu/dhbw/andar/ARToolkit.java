...
//					for (int j = 0; j<16; j++){
//						Log.i("float " + Integer.toString(j), Float.toString(obj.glMatrix[j]));
//					}
					
					
					// set passed invariable to 
					coords[0] = obj.glMatrix[13];
					coords[1] = obj.glMatrix[12];
					mname = obj.name;
					newval = true;
					...
					Log.i("Name", mname);
	/**				
					if(obj.glMatrix[13] < xmin)
						xmin = obj.glMatrix[13];
					if(obj.glMatrix[13] > xmax)
						xmax = obj.glMatrix[13];
					if(obj.glMatrix[12] < ymin)
						ymin = obj.glMatrix[12];
					if(obj.glMatrix[12] > ymax)
						ymax = obj.glMatrix[12];
					
					
					

					Log.i("float (xmin)", Float.toString(xmin));
					Log.i("float (xmax)", Float.toString(xmax));
					Log.i("float (ymin)", Float.toString(ymin));
					Log.i("float (ymax)", Float.toString(ymax));
					
					**/
//					Log.i("float 12 (y)", Float.toString(obj.glMatrix[12]));
//					Log.i("float 11", Float.toString(obj.glMatrix[11]));
			Log.i("tracking", mname);		
		//			Thread.dumpStack();
